 ...
 * in funzioni... Comunque visto che vogliono cos�.. amen! */
Info getArgs(int argc, char*argv[]);
void bubbleSort(int, int*);/* Le bolle pi� grandi vengono a galla */
...
void bubbleSort(int i, int* v){ /* Che poi su wikipedia ti spiega magari anche il perch� si fa cos�.. */
	int j;/* Bah.. il senso di darci gi� il codice pronto e farcelo tradurre... MISTERO! xD */
	for (i--; i > 0; i--){
		for( j = 0; j < i; j++)
			if(v[j] > v[j+1]){
				swap(v+j, v+1+j);
